...
- **Panel Dark** (`#0a0a0a`): Sidebar and panel backgrounds. Studio variant — flatter, closer to marketing black than Linear's original `#0f1011`.
...
### Border & Divider
- **Border Standard** (`#262626`): Studio variant — solid dark border for cards, inputs, nav chrome, and prominent separations. Replaces Linear's semi-transparent white borders.
- **Border Solid** (`#262626`): Alias of standard; exists for components that previously distinguished the two.
- **Border Subtle** (`rgba(255,255,255,0.05)`): Ultra-subtle semi-transparent border for inner dividers and whisper-thin separations.
...
**Sharp corners everywhere.** The Orca Studio variant of this design system
deliberately departs from Linear on this single point: all rectangular
surfaces — buttons, inputs, cards, dialogs, popovers, pills, badges, nav
rows, dropdowns — use a **0px corner radius**. No `rounded-md`, no
`rounded-lg`, no pills. This gives the UI a more engineered, schematic
character that pairs naturally with a node-graph editor.

The only exceptions are **functional circles** — elements whose roundness is
semantic, not decorative:

- React Flow connection handles (50%) — they're connection points, not corners
- Slider thumbs (50%) — drag affordances read as dots
- Status/presence dots (50%) — conventional circular indicator

Everything else is square. If you find yourself reaching for `rounded-*`
outside the above list, stop — it's out of spec.
...
- Don't add rounded corners to rectangular elements — the Studio variant is sharp-cornered everywhere; only 50% circles (handles, slider thumbs, status dots) are permitted
