...
Every Orca construct is a \textbf{block}: a keyword identifying the block
type, a user-chosen name, and a brace-delimited body of key-value
assignments:
...
Orca defines seven block types, each corresponding to a component of an
...
\noindent\textbf{String handling.} The lexer supports escape sequences
(\texttt{\textbackslash n}, \texttt{\textbackslash t},
\texttt{\textbackslash\textbackslash},
\texttt{\textbackslash"}), line continuation with trailing
\texttt{\textbackslash}, and multi-line string auto-dedenting. For
multi-line strings, the baseline indentation is determined by the column
position of the closing quote, and leading whitespace up to that column
is stripped from each line. This allows developers to write:
...
\noindent without the indentation leaking into the string value.
...
\noindent\textbf{Expression parsing.} The Pratt parser handles six
