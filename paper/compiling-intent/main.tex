...
orchestration, which produces \texttt{.oc} source files that are then
...
\texttt{.oc} source files, which are then compiled by the Orca compiler
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Stage 1 (non-deterministic):} Natural language
        $\xrightarrow{\text{agentic compiler}}$ \texttt{.oc} files.
        This stage uses LLM agents and is inherently non-deterministic.
  \item \textbf{Stage 2 (deterministic):} \texttt{.oc} files
...
\texttt{orca.oc} once the full pipeline is implemented.
\end{todobox}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Learning from corrections.} When a user manually fixes
        generated \texttt{.oc} code, feeding corrections back to
