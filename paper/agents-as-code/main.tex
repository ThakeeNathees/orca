...
the original \texttt{.orca} source, enabling debugging at the DSL level.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Source-mapped code generation} that annotates every line of
        generated Python with its originating \texttt{.orca} source location,
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Debuggability.} Generated code carries source mappings back
        to the \texttt{.orca} file, so developers debug in terms of their
...
embedded \texttt{builtins.orca} file, parsed at compiler initialization:
...
\begin{center}
\texttt{.orca} $\xrightarrow{\text{Lexer}}$ Tokens
...
\begin{itemize}
  ...
  \item Source mapping annotations: every generated line carries a comment
        \texttt{\# source.orca:L:C} tracing back to the originating
        declaration.
...
# --- Models ---          # agents.orca:1:1
gpt4 = ChatOpenAI(        # agents.orca:1:1
    model="gpt-4o",       # agents.orca:3:3
    temperature=0.2,       # agents.orca:4:3
)

# --- Nodes ---
def researcher_node(state):  # agents.orca:11:1
    """You research topics thoroughly..."""
    pass  # tool-bound agent invocation

def writer_node(state):      # agents.orca:20:1
    """You are a technical writer."""
    pass  # agent invocation

# --- Graph ---              # agents.orca:28:1
...
\begin{enumerate}
  ...
  \item Tracing back to the originating \texttt{.orca} declaration.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Visual editor.} A node-and-edge graph editor that generates
        \texttt{.orca} files, providing a visual frontend for users who prefer
  ...
  \item \textbf{Runtime debugging.} Integrating source maps with a debugger
        that allows breakpoints and stepping in terms of \texttt{.orca}
        source lines.
